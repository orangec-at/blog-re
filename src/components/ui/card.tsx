...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn("rounded-lg border border-rule bg-paper text-ink", className)}
      data-slot="card"
      {...props}
    />
  );
...
function CardTitle({ className, ...props }: React.ComponentProps<"h3">) {
  return (
    <h3
      className={cn("font-display text-2xl leading-tight tracking-[-0.035em] text-ink", className)}
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return <div className={cn("text-sm leading-6 text-ink", className)} data-slot="card-description" {...props} />;
}
